...
def handle_response(response):
    ...
    Logger.log(f'Processed frames # {frame_seq_no}')
...
def log_metrics(split_point, flops, time, single_frame_time, left_output_size, avg_consec_inference_gap, total_left_model_time):
    ...
    write_to_csv('customized_model_async' + '.csv', metrics_headers, [frames_to_process, split_point, flops, time, single_frame_time, left_output_size, avg_consec_inference_gap, total_left_model_time, total_right_model_time])
...
async def consumer():
    ...
    async for item in q:
        try:   
            ...
            post_data = {'client_id': "client_4",'data': item[0], 'frame_seq_no': item[1]} # item[0] = out_left, item[1] = frame_seq_no
...
def get_request_body(left_output, frame_seq_no, split_point):
    # Request JSON.
    request_body = {'client_id': "client_4",'data': left_output, 'frame_seq_no': frame_seq_no, 'split_point': split_point}
    return request_body
